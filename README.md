...
4G Dongle 固件在 LuCI「状态 → 概览」内置设备温度模块，随概览状态轮询刷新。动态读取内核 thermal_zone 的类型和毫摄氏度读数，显示 CPU、调制解调器、GPU、PM8916 等可用传感器；缺失或读取失败时显示不可用，不将失败读数当成 0°C。camera-thermal 是芯片热区名称，不表示设备装有摄像头。仅授予温度文件的读取权限，不修改温控阈值。当前 MSM8916 棒子已实测显示。
